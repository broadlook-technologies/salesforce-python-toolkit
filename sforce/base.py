...
logger = getLogger('SalesforcePythonToolkit')
...
Salesforce will use HTTPS.')
    ...
    def generateHeader(self, sObjectType):
        ...
        try:
            ...
            logger.warning('There is not a SOAP header of type %s' % sObjectType)
    ...
    def _marshallSObjects(self, sObjects, tag='sObjects'):
        ...
        if not isinstance(sObjects, (tuple, list)):
            sObjects = (sObjects,)
        nsPrefix = 'tns:' if sObjects[0].type in ['LeadConvert', 'SingleEmailMessage', 'MassEmailMessage'] else 'ens:'
    ...
    def _setHeaders(self, call=None):
        ...
        if self._allowFieldTruncationHeader is not None and call in ('convertLead', 'create', 'merge', 'process',
                                                                     'undelete', 'update', 'upsert'):
            headers['AllowFieldTruncationHeader'] = self._allowFieldTruncationHeader

        if self._assignmentRuleHeader is not None and call in ('create', 'merge', 'update', 'upsert'):
            headers['AssignmentRuleHeader'] = self._assignmentRuleHeader

        # CallOptions will only ever be set by the SforcePartnerClient
        if self._callOptions is not None and call in ('create', 'merge', 'queryAll', 'query', 'queryMore',  'retrieve',
        ...
        if self._emailHeader is not None and call in ('create', 'delete', 'resetPassword', 'update', 'upsert'):
            headers['EmailHeader'] = self._emailHeader

        if self._localeOptions is not None and call in ('describeSObject', 'describeSObjects'):
            headers['LocaleOptions'] = self._localeOptions

        if self._loginScopeHeader is not None and call == 'login':
            headers['LoginScopeHeader'] = self._loginScopeHeader

        if self._mruHeader is not None and call in ('create', 'merge', 'query', 'retrieve', 'update', 'upsert'):
            headers['MruHeader'] = self._mruHeader

        if self._packageVersionHeader is not None and call in ('convertLead', 'create', 'delete', 'describeGlobal',

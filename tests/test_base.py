...
class SforceBaseClientTest(unittest.TestCase):
    ...
    def checkHeaders(self, call):
        ...
        if self.wsdlFormat == 'Partner' and call in ('create', 'merge', 'queryAll', 'query', 'queryMore', 'retrieve',
                                                     'search', 'update', 'upsert', 'convertLead', 'login', 'delete',
                                                     'describeGlobal', 'describeLayout', 'describeTabs',
                                                     'describeSObject', 'describeSObjects', 'getDeleted', 'getUpdated',
                                                     'process', 'undelete', 'getServerTimestamp', 'getUserInfo',
        ...
        if call in ('convertLead', 'create', 'delete', 'describeGlobal', 'describeLayout', 'describeSObject',
                    'describeSObjects', 'describeTabs', 'merge', 'process', 'query', 'retrieve', 'search', 'undelete',
                    'update', 'upsert'):
    ...
    def createLead(self, return_lead=False):
    ...
    def createLeads(self, return_leads=False):
        lead = self.h.generateObject('Lead')
        lead.FirstName = u'Joë'
        lead.LastName = u'Möke'
        lead.Company = u'你好公司'
    ...
    def testConvertLead(self):
        ...
        lead_convert = self.h.generateObject('LeadConvert')
        lead_convert.leadId = result.id
        lead_convert.convertedStatus = 'Qualified'
        result = self.h.convertLead(lead_convert)
    ...
    def testCreateCustomObject(self):
        ...
        case_note = self.h.generateObject('Case_Note__c')
        case_note.case__c = result.id
        case_note.subject__c = 'my subject'
        case_note.description__c = 'description here'
        result = self.h.create(case_note)
    ...
    def testMerge(self):
        ...
        merge_request = self.h.generateObject('MergeRequest')
        merge_request.masterRecord = lead
        merge_request.recordToMergeIds = result[1].id
        result = self.h.merge(merge_request)
    ...
    def testProcessSubmitRequestMalformedId(self):
        ...
        process_request = self.h.generateObject('ProcessSubmitRequest')
        process_request.objectId = '*ID OF OBJECT PROCESS REQUEST AFFECTS*'
        process_request.comments = 'This is what I think.'
        result = self.h.process(process_request)
    ...
    def testProcessSubmitRequestInvalidId(self):
        process_request = self.h.generateObject('ProcessSubmitRequest')
        process_request.objectId = '00Q000xxxxxxxxx'
        process_request.comments = 'This is what I think.'
        result = self.h.process(process_request)
    ...
    def testProcessWorkitemRequestMalformedId(self):
        process_request = self.h.generateObject('ProcessWorkitemRequest')
        process_request.action = 'Approve'
        process_request.workitemId = '*ID OF OBJECT PROCESS REQUEST AFFECTS*'
        process_request.comments = 'I approved this request.'
        result = self.h.process(process_request)
    ...
    def testProcessWorkitemRequestInvalidId(self):
        process_request = self.h.generateObject('ProcessWorkitemRequest')
        process_request.action = 'Approve'
        process_request.workitemId = '00Q000xxxxxxxxx'
        process_request.comments = 'I approved this request.'
        result = self.h.process(process_request)
  ...
  "<?xml version="1.0" encoding="UTF-8"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns="urn:enterprise.soap.sforce.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sf="urn:sobject.enterprise.soap.sforce.com"><soapenv:Body><queryAllResponse><result><done>true</done><queryLocator xsi:nil="true"/><records xsi:type="sf:Contact"><sf:Account xsi:type="sf:Account"><sf:Name>Unknown</sf:Name></sf:Account><sf:LastName>Administrator</sf:LastName></records><size>1</size></result></queryAllResponse></soapenv:Body></soapenv:Envelope>"
    ...
    def testQueryMore(self):
        ...
        while result.done is False:
            self.assertTrue(result.queryLocator is not None)
    ...
    def testDescribeGlobal(self):
        ...
        found_account = False
        for s_object in result.sobjects:
            if s_object.name == 'Account':
                found_account = True

        self.assertTrue(found_account)
